...
public class FullJoinApp {
    ...
    public static void main(String[] args) {
        ...
        JavaPairRDD<Integer, Tuple2<Optional<Integer>, Optional<String>>> joined = visits.fullOuterJoin(users);

        joined.collect().forEach(System.out::println);

        joined.collect().forEach(
                it -> {System.out.println(it._2._1.orElse(0) + "--" + it._2._2.orElse("No name"));
        });
